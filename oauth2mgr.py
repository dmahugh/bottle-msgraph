"""OAuth2Manager class definition. This class handles the details of obtaining
an authorization code and access token from Azure Active Directory, and provides
wrappers for making authenticated calls to the Microsoft Graph API.

Dependencies: Bottle, Requests
"""
import json
import os
import time
...
class OAuth2Manager(object): #-----------------------------------------------<<<
    """Class for managing OAuth2 process flow."""
    def __init__(self, configfile='config.json'):
        ...
        self.cache('read')
        self.after_login = '/' # default is to redirect to home page after login
    ...
    def cache(self, action): #-----------------------------------------------<<<
        ...
        if action == 'save':
            ...
            if os.path.isfile(cachefile):
                ...
                self.token_value = configdata['token_value']
    ...
    def default_headers(self): #---------------------------------------------<<<
        ...
        return {'User-Agent' : 'bottle-msgraph/1.0',
                'Authorization' : 'Bearer {0}'.format(self.token_value),
    ...
    def get_token(self, authcode): #-----------------------------------------<<<
        ...
        self.token_value = jsondata['access_token']
        ...
        print('SUCCESS: OAuth2 access token acquired ({0} bytes)'. \
            format(len(self.token_value)))
    ...
    def token_abbrev(self, token_val=None, type='access'): #-----------------<<<
        ...
        type == 'access' (default) - self.token_value
        type == 'refresh' - self.refresh_token
        """
        if not token_val:
            if type.lower() == 'refresh':
    ...
    def token_seconds(self): #-----------------------------------------------<<<
        ...
        if not self.token_value or time.time() >= self.token_expires_at:
            return 0
