...
def contacts():
    ...
    return dict(status_code=response.status_code, data=jsondata, nextlink=nextpage_url)
...
def orginfo():
    ...
    # so current user is not in an organization
    return dict(
        org_id=None,
        org_name=None,
        status_code=response.status_code,
...
def print_env(title, req_obj=None, oauthmgr=None):
    ...
    if oauthmgr:
        ...
        print(
            "token_expires_at:  {0} ({1:.0f} seconds from now)".format(
                oauthmgr.token_expires_at, oauthmgr.token_seconds()
            )
        )
